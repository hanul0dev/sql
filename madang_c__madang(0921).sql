--박지성이 구매한 도서의 출판사와 같은 출판사에서 도서를 구매한 고개의 이름을 출력하시오.
...
--새로운 도서(스포츠세계,대한미디어,10000)이 마당서점에 입고되었다.
--데이터 행을 추가하시오.
--만약 삽입이 되지 않는다면 부족한 데이터가 있는지 확인해본다.
insert into book values(15,'스포츠 세계','대한미디어',10000);
...
--무결성 제약조건이 위배되었습니다~ orders 테이블의 자식 레코드가 위배되었습니다.
...
select orderid, orderdate 주문일,orderdate+10 주문확정일 from orders;

--2023/8/28에 주문 받은 주문번호, 주문일, 고객번호, 도서번호를 모두 출력하시오.
...
--2023/8/28에 주문 받은 주문번호, 주문일, 고객명, 도서명를 모두 출력하시오.
--단, 주문일은 "yyyy년 mm월 dd일"과 같은 format으로 표시하시오.
select orderid 주문번호, to_char(orderdate,'yyyy-mm-dd') 주문일, name 고객명, bookname 도서번호
from orders o,book b,customer c
where c.custid=o.custid and o.bookid=b.bookid
and orderdate=to_date('20230828','yyyymmdd');

--DB서버의 설정된 날짜와 시간 요일을 출력하시오.
--실제 반환도닌 날짜와 형식을 지정한(년도4자리/월2자리/일2자리 간략한 요일 , 시:분:초) 날짜와 시간 2가지 출력
