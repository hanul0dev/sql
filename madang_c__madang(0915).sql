...
--주문정보(neworders 테이블)
--orderid(기본키)
--orderid, custid(not null,customer custid 참조해서 외래키, 연쇄삭제) 
--bookid(not null,newbook1 bookid 참조 외래키, 연쇄 삭제) 
--saleprice
--orderdate(date형)
...
--bookanme 컬럼의 varchar2(30) not null 제약조건 변경
alter table newbook1
...
update customer set address='부산광역시' where name='박세리';

--박세리 고객의 주소를 김연아 고객의 주소와 동일하게 변경하시오.
update customer set address=(select address from customer where name='김연아') where name='박세리';

--박세리 고객의 전화번호를 김연아 고객의 주소와 동일하게 변경하시오.
update customer set phone=(select phone from customer where name='김연아') where name='박세리';
